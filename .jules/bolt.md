...
## 2026-04-05 - Memoizing derived state in custom hooks
**Learning:** Custom hooks that compute values from state and return new object references on every render (e.g., using `Object.values`, `reduce`, or `some`) cause unnecessary re-renders in all consuming components because reference equality checks fail.
**Action:** Always wrap derived state calculations and object transformations in custom hooks with `useMemo`, using the underlying state variables as dependencies, to ensure stable object references.
