...
socialRouter.get('/platforms', (_req, res) => {
...
socialRouter.get('/auth/:platform', (req, res) => {
...
socialRouter.post('/auth/callback', async (req, res) => {
    const { code, state } = req.body;
    if (!code || !state) {
        return res.status(400).json({ error: 'Missing code or state parameter' });
    }
    const [platform, stateId] = state.split(':');
    const config = getPlatformConfig(platform);
    if (!config) {
    ...
    try {
        ...
        if (platform === 'instagram') {
            ...
            tokenResponse = await axios_1.default.post(config.tokenUrl, new URLSearchParams(tokenParams), {
        ...
        if (platform === 'instagram' && tokens.access_token) {
            try {
                const longLivedResp = await axios_1.default.get(`${config.apiBase}/access_token`, {
                    params: {
        ...
        if (platform === 'facebook' && tokens.access_token) {
            try {
                ...
                const longLivedResp = await axios_1.default.get(longLivedUrl);
...
socialRouter.post('/fetch/:platform', async (req, res) => {
    const platform = req.params.platform;
...
socialRouter.post('/refresh/:platform', async (req, res) => {
    const platform = req.params.platform;
    ...
    try {
        ...
        const response = await axios_1.default.post(tokenUrl, new URLSearchParams(params), {
            headers: { 'Content-Type': 'application/x-www-form-urlencoded' },
        });
        res.json({
...
function getDefaultApiBase(platform) {
    const bases = {
