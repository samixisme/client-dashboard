...
### 4. Production Server Rules (Engintron, CORS, SSL)

- **Playbook First:** Before diagnosing or deploying new subdomains on the VPS, you **MUST** read `./SERVER_PLAYBOOK.md` in the project root.
- **Engintron Hacks:** Nginx is completely managed by Engintron. Never edit `default_https.conf`. Always append rules to `/etc/nginx/conf.d/custom_rules.conf`.
- **Next.js Routing:** Always pass `X-Forwarded-Proto https` and `X-Forwarded-SSL on` to PM2/Next.js upstream servers or you will trigger infinite redirect loops.
- **SSL Automation:** Check the playbook for the specific `sed` script needed to bypass cPanel's fake certificate generation if `certbot` fails.
