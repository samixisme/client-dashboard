...
export function useProjectFiles(projectId: string | undefined): ProjectFile[] {
  ...
  return useMemo(() => {
    ...
    if (driveFiles && driveFiles.length > 0) {
      const mappedDrive = driveFiles.map(
        (file): ProjectFile => ({
          id: `drive:${file.id}`,
          projectId,
          name: file.name,
    ...
    // 2. Project Links
    const links = data.projectLinks.filter(
    ...
    // 3. Task Attachments
    // ⚡ Bolt: Use a Set for O(1) lookups instead of O(N*M) Array.includes()
    ...
    tasks.forEach((task) => {
      if (task.attachments && task.attachments.length > 0) {
        const attachs = task.attachments.map(
          (att): ProjectFile => ({
    ...
    // 4. Feedback Mockups
    const mockups = data.feedbackMockups.filter(
      (m) => m.projectId === projectId,
    );
    mockups.forEach((mockup) => {
      if (mockup.images && mockup.images.length > 0) {
        const imgs = mockup.images.map(
          (img): ProjectFile => ({
