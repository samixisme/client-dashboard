...
export function useProjectFiles(projectId: string | undefined): ProjectFile[] {
  ...
  return useMemo(() => {
    ...
    // 3. Task Attachments
    // Optimization: Pre-compute a Set for O(1) lookups instead of O(N) Array.includes()
    const projectBoardIdsSet = new Set(
      data.boards
        .filter(b => b.projectId === projectId)
        .map(b => b.id)
    );
      
    const tasks = data.tasks.filter(t => projectBoardIdsSet.has(t.boardId));
    tasks.forEach(task => {
