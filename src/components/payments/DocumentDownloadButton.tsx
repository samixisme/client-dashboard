...
export const DocumentDownloadButton: React.FC<DocumentDownloadButtonProps> = ({
    ...
    const buttonClasses = variant === 'primary'
        ? 'inline-flex items-center justify-center gap-2 px-4 py-2 bg-primary text-white font-semibold rounded-lg hover:bg-primary-hover hover:scale-105 transition-all duration-300 shadow-md hover:shadow-lg disabled:opacity-50 disabled:cursor-not-allowed'
    ...
    return (
        <button
            ...
            aria-label={isGenerating ? 'Generating document' : 'Download document'}
        >
            {variant === 'secondary' ? (
                isGenerating ? (
