...
/**
 * Converts a text string and match positions into an array of objects
 * with isMatch flags.
 *
...
export function highlightMatches(
  ...
  for (const range of merged) {
    ...
    if (start > lastEnd) {
      result.push({ text: croppedText.slice(lastEnd, start), isMatch: false });
    }
    result.push({ text: croppedText.slice(start, end), isMatch: true });
    lastEnd = end;
  }

  if (lastEnd < croppedText.length) {
    result.push({ text: croppedText.slice(lastEnd), isMatch: false });
  }
