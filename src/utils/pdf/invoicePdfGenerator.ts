...
import { Invoice, Client, UserSettings, LineItem } from '../../../types';
// @ts-ignore - Image import
import backgroundImage from '../../templates/invoice/invoice-background.png';
// @ts-ignore - Image import
import termsBackgroundImage from '../../templates/invoice/invoicepageterms.png';
...
export class InvoicePdfGenerator {
     ...
     */
    private static getAsteriskNumber(invoice: Invoice, itemId: string): number {
        let counter = 0;
        for (const category of invoice.itemCategories) {
     ...
     */
    private static async renderTermsPage(pdf: jsPDF, invoice: Invoice, asteriskItems: LineItem[]): Promise<void> {
        // Setup fonts
        pdf.setFont('SpaceGrotesk', 'normal');
        pdf.setFontSize(8);
        ...
        asteriskItems.forEach((item, index) => {
            ...
            const asteriskNum = this.getAsteriskNumber(invoice, item.id);
    ...
    /**
     * Generate and download invoice PDF with background image
     */
    static async generateInvoicePdf(
        ...
        try {
            ...
            // Invoice Number - Pixel coords: (838, 141)
            const invoiceNumPos = this.pxToMm(838, 141);
            const invoiceNumText = invoice.invoiceNumber.toUpperCase();
            pdf.text(invoiceNumText, invoiceNumPos.x, invoiceNumPos.y);
            ...
            const category = invoice.itemCategories[0];
            ...
            items.slice(0, 7).forEach((item, index) => {
                ...
                if (item.hasAsterisk) {
                    const asteriskNumber = this.getAsteriskNumber(invoice, item.id);
            ...
            const totalNet = invoice.totals?.totalNet || 0;
            ...
            const asteriskItems = invoice.itemCategories
            ...
            if (asteriskItems.length > 0) {
                ...
                await this.renderTermsPage(pdf, invoice, asteriskItems);
            }
