...
describe('urlValidator', () => {
  describe('validateUrl', () => {
    ...
    it('blocks IPv6 unspecified :: (0.0.0.0 equivalent)', () => {
      const result = validateUrl('http://[::]:8080');
      expect(result.isValid).toBe(false);
    });

    it('blocks expanded IPv6 loopback', () => {
      const result = validateUrl('http://[0:0:0:0:0:0:0:1]');
      expect(result.isValid).toBe(false);
    });

    it('blocks IPv4-mapped IPv6 loopback (decimal)', () => {
      const result = validateUrl('http://[::ffff:127.0.0.1]');
      expect(result.isValid).toBe(false);
    });

    it('blocks IPv4-mapped IPv6 private (decimal)', () => {
      const result = validateUrl('http://[::ffff:192.168.1.1]');
      expect(result.isValid).toBe(false);
    });

    it('blocks IPv4-mapped IPv6 loopback (hex)', () => {
