...
import { FeedbackComment, FeedbackMockup, FeedbackWebsite, FeedbackVideo, MockupImage, DeviceView, BoardMember, VideoAsset } from '../types';
...
const FeedbackItemPage = () => {
    ...
    const handleResolveComment = (commentId: string) => {
        ...
        const globalComment = data.feedbackComments.find(c => c.id === commentId);
        if(globalComment) globalComment.status = 'Resolved';
        handleClosePopover();
    };
                ...
                <div className="flex-1 flex flex-col gap-4 overflow-hidden">
                    <div ref={viewerContainerRef} className="flex-1 bg-glass rounded-lg border border-border-color overflow-auto flex justify-center items-center p-4 relative gap-4">
                        <div 
                            ...
                            className={`relative flex-shrink-0 transition-transform ${interactionMode === 'comment' && !isVideo ? 'cursor-crosshair' : ''} ${isVideo && isPlacingVideoComment ? 'cursor-crosshair' : ''}`} 
                            style={contentStyle}
                        >
                            ...
                            {(pinsVisible && (isMockup || (isWebsite && interactionMode === 'comment'))) && pinsForCurrentView.map(comment => (
                                <div key={comment.id} className="comment-pin absolute group" style={{ left: `${comment.x_coordinate}px`, top: `${comment.y_coordinate}px`, transform: `scale(${1/zoom})` }} onClick={(e) => handlePinClick(comment, e)}>
                                    <div className={`h-8 w-8 rounded-full flex items-center justify-center text-background font-bold text-sm ring-4 ring-background/50 cursor-pointer shadow-lg transition-transform group-hover:scale-110 ${comment.status === 'Resolved' ? 'bg-green-500' : 'bg-primary'}`}>
                                        {comment.pin_number}
                                    </div>
                                </div>
                            ))}
