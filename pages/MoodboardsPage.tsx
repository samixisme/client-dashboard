...
const MoodboardsPage = () => {
  ...
  return (
    <div>
      <h1 className="text-3xl font-bold text-text-primary">{brand ? `Moodboards for ${brand.name}` : 'Moodboards'}</h1>
      <p className="mt-2 text-text-secondary">Select a project to view its moodboards.</p>

      <div className="mt-8 grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
        {filteredProjects.length > 0 ? filteredProjects.map(project => (
          <Link 
            key={project.id} 
            to={`/moodboards/${project.id}`}
