            ...
            <div className="mt-2 space-y-1">
                {(editedBrand.imagery || []).map((image, index) => (
                     <div key={index} className="flex items-center justify-between bg-glass-light p-2 rounded-md text-sm">
                        ...
                        <button type="button" onClick={() => { handleUpdate('imagery', (editedBrand.imagery || []).filter((_, i) => i !== index)); toast.success('Image removed'); }} className="p-1 hover:text-red-500" aria-label="Remove imagery" title="Remove imagery"><DeleteIcon className="h-4 w-4"/></button>
                    </div>
                ))}
            </div>
        </div>
