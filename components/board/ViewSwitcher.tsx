    ...
    return (
        <div ref={wrapperRef} className="relative z-50">
            <button
                ...
                aria-haspopup="true"
                aria-label={`Current view: ${activeView.name}`}
            ...
            {isOpen && (
                <div role="menu" className={`absolute top-full right-0 mt-2 ${widthClass} bg-glass/95 backdrop-blur-xl rounded-xl shadow-2xl border border-border-color z-[9999] p-2 animate-scale-in`}>
                    {options.map(option => (
                        <button
                            key={option.id}
                            role="menuitem"
