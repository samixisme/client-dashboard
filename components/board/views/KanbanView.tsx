                        ...
                        <div key={stage.id} className="w-80 flex-shrink-0 flex flex-col bg-glass/40 backdrop-blur-xl rounded-xl border border-border-color shadow-xl">
                            <div className="flex justify-between items-center p-3 rounded-t-xl" style={{...pattern}}>
                                ...
                                <div className="flex items-center gap-2 text-text-secondary">
                                    <button onClick={() => setAddingToStage(stage.id)} aria-label={"Add task to " + stage.name} title="Add task" className="hover:text-primary hover:bg-glass-light/60 hover:scale-110 transition-all duration-300 p-1 rounded-lg hover:shadow-lg"><AddIcon className="h-5 w-5"/></button>
                                    <button onClick={(e) => onOpenStageActions(e.currentTarget, stage)} aria-label="Stage settings" title="Stage settings" className="hover:text-primary hover:bg-glass-light/60 hover:scale-110 transition-all duration-300 p-1 rounded-lg hover:shadow-lg"><SettingsIcon className="h-5 w-5"/></button>
                                </div>
                            </div>
                                        ...
                                        {addingToStage === stage.id ? (
                                            <div className="mt-3 p-1">
                                                ...
                                                <div className="flex items-center gap-2 mt-2">
                                                    ...
                                                    <button onClick={() => setAddingToStage(null)} aria-label="Cancel adding task" title="Cancel" className="text-xl text-text-secondary hover:text-text-primary leading-none hover:scale-110 transition-all duration-300">&times;</button>
                                                </div>
                                            </div>
                                        ) : (
