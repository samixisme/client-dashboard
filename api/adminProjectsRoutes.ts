...
router.post('/', async (req: Request, res: Response) => {
  try {
    ...
    if (data.memberIds.length > 0) {
      // ⚡ Bolt Optimization: Use db.getAll() for batched network requests instead of N separate doc().get() calls
      // This reduces N+1 queries and significantly improves performance for larger member arrays
      const memberRefs = data.memberIds.map((uid) => db.collection('users').doc(uid));
