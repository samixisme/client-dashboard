...
router.post('/', async (req: Request, res: Response) => {
  try {
    ...
    if (data.memberIds.length > 0) {
      // ⚡ Bolt: Fetch all docs to validate transitions using efficient batched db.getAll() instead of Promise.all
      const userRefs = data.memberIds.map((uid) => db.collection('users').doc(uid));
      const memberChecks = await db.getAll(...userRefs);
